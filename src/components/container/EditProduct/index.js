...
const EditProduct = ({
  ...
  useEffect(() => {
    if (productId !== '' && Array.isArray(allProducts) && allProducts.length && allProducts.length > 0) {
      let selectedProduct = allProducts.find((x) => x.id === productId);
      if (selectedProduct) {
        setValueToInput(selectedProduct);

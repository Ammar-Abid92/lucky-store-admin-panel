...
const ViewOrders = ({ user, orders, history, location }) => {
  ...
  const handleSearch = (text) => {
    ...
    //     ...orders.filter(
    //       (x) =>
    //         x.status.includes(selectedFilter.toLowerCase()) &&
    //         (x.customer.name.toLowerCase().includes(text.toLowerCase()) ||
  ...
  const shareStore = () => {
    ...
    analytics.logEvent('share_from_orders_screen');
              ...
              <div className='RightSectionMain'>
                ...
                {orders.length > 0 ? (
                  <>
                    {loading == true ? (
                      ...
                      <>
                              ...
                              <tbody>
                                {allOrders.length
                                  ? allOrders.map((order, index) => {
                                    return <ListItem key={order._id} order={order} index={index} activePage={activePage} history={history} user={user} allOrders={allOrders} />;
                                  })
                                  : null}
                              </tbody>
                            </table>
                        ...
                        {/* {orders.length > 5 && allOrders.length ? (
                          <div className='showMoreButton'>
                            <button className='btn btn-primary login_btn_next' onClick={() => handleLoadMore(allOrders.length === orders.length ? false : true)}>
                              {allOrders.length === orders.length ? 'show less' : 'show more'}
                            </button>
                          </div>
                        ) : null} */}

                        {!allOrders.length && search ? (
                          <div className='placeHolderContMain'>
                            <p>
                              <i className='orderPlaceholderIcon'></i>
                            </p>
                            <p>No orders found</p>
                            <p className='placeholderParagraph'>You have not received any orders yet</p>
                          </div>
                        ) : null}
                      </>
                    )}
                  ...
                  <div className='placeHolderContMain'>
                    ...
                    <button className="storeShare login_btn_next marTop5" onClick={shareStore}>
                      <span>
